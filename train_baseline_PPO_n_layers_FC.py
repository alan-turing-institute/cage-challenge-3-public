...
# Train the PPO baseline on the standard CybORG observation space (paper Table 2; model M1).
...
def env_creator_CC3(env_config: dict):
    ...
    env = MAPPODivRewParallelPettingZooEnv(PettingZooParallelWrapper(env=cyborg))
    #env = EnvCompatibility(cyborg) # Convert to Gymnasium from gym
    return env
